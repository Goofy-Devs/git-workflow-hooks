...
def main():
    ...
    if conflict_output:
        ...
        print(f"To resolve conflicts, merge or rebase:")
        print(f"{hu.INDENT}* Merge: {hu.HIGHLIGHTS.get('reverse', '')}git fetch origin && git merge origin/main{hu.TEXT_RESET}")
        print(f"{hu.INDENT}* Rebase: {hu.HIGHLIGHTS.get('reverse', '')}git fetch origin && git rebase origin/main{hu.TEXT_RESET}")
        print(f"After resolving the conflicts, push your changes to origin:")
        print(f"{hu.INDENT}* After merge: {hu.HIGHLIGHTS.get('reverse', '')}git push -u origin {branch_name}{hu.TEXT_RESET}")
        print(f"{hu.INDENT}* After rebase: {hu.HIGHLIGHTS.get('reverse', '')}git push -u origin {branch_name} --force-with-lease{hu.TEXT_RESET}")
